...
form.onsubmit=async e=>{e.preventDefault();msg.textContent="";submit.disabled=true;try{const email=$("#email").value.trim(),password=$("#password").value;if(mode==="signup"){const name=$("#name").value.trim();const{data,error}=await supabase.auth.signUp({email,password,options:{emailRedirectTo:HUB_WEB_URL,data:name?{name}:undefined}});if(error)throw error;if(!data.session)msg.textContent="Аккаунт создан. Откройте письмо и подтвердите email.";else render(data.session)}else{const{data,error}=await supabase.auth.signInWithPassword({email,password});if(error)throw error;render(data.session)}}catch(e){msg.textContent=e?.message||"Ошибка авторизации"}finally{submit.disabled=false}};
