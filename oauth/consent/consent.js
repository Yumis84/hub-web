...
if(!id){fail("Некорректный OAuth-запрос: отсутствует authorization_id");}
 ...
 if(!access_token||!refresh_token){
  ...
  try{
   const{error:se}=await sb.auth.setSession({access_token,refresh_token});if(se)throw se;
   ...
   $("#approve").onclick=async()=>{try{$("#approve").disabled=true;const{data,error}=await sb.auth.oauth.approveAuthorization(id);if(error)throw error;location.assign(data.redirect_url)}catch(e){fail(e.message)}};
